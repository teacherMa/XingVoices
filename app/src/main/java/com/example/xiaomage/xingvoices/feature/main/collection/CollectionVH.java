    ...
    public void onBottomCommentItemClick(int position, String content) {
        switch (position) {
            case Constants.BottomComItem.START_RECORD:
                EventBus.getDefault().post(new VHRecordEvent(Constants.ViewHolderTag.CollectionVH, true));
                break;
            case Constants.BottomComItem.STOP_RECORD:
                EventBus.getDefault().post(new VHRecordEvent(Constants.ViewHolderTag.CollectionVH, false));
                break;
            case Constants.BottomComItem.TO_AUDITION:
                EventBus.getDefault().post(new VHAuditionEvent(Constants.ViewHolderTag.CollectionVH));
                break;
            case Constants.BottomComItem.STOP_AUDITION:
                EventBus.getDefault().post(new VHAuditionEvent(Constants.ViewHolderTag.CollectionVH));
                break;
            case Constants.BottomComItem.SEND_TEXT_COM:
                EventBus.getDefault().post(new VHPublishTextComEvent(Constants.ViewHolderTag.CollectionVH,
                        content, mRemoteVoice));
                break;
            case Constants.BottomComItem.SEND_VOICE_COM:
                EventBus.getDefault().post(new VHPublishVoiceComEvent(Constants.ViewHolderTag.CollectionVH, mRemoteVoice, content));
                break;
            default:
                break;
        }
